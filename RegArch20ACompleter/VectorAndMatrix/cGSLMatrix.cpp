...
static cGSLVector ourTempVector = cGSLVector();
static cGSLMatrix ourTempMatrix = cGSLMatrix();
...
//		#endif // _RDLL_
	...
	*/
	...
	uint myVectNRow = theVect.GetSize() ;
		...
		for (int i = 0 ; i < (int)myVectNRow ; i++)
			gsl_matrix_set(mvMat, i, 0, theVect[i]) ;
		return *this ;
	}
...
/*!
	...
	*/
	...
	cGSLMatrix mySInvMat = Diag(myS) ;
		...
		ourTempMatrix = myV * mySInvMat * Transpose(myU);
		return ourTempMatrix ;
	}
